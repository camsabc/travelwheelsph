    ...
    const fetchData = async () => {
      if (email) {
        try {
          ...
          const ridesResponse = await fetch(`hhttps://travelwheelsph.onrender.com/api/rides/get-all-rides`);
          ...
          <MDBRow>
            {rides.map((ride) => (
              <MDBCol md="6" lg="4" className="mb-4 align-items-center" key={ride._id}>
                <div
                  onClick={() => navigate(`/educ2/${ride._id}`, { state: { email: user?.email, rideName: ride.name } })}
