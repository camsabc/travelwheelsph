...
<div className="my-4" style={{ width: '100%', overflow: 'hidden', display: 'flex', flexWrap: 'wrap', justifyContent: 'center' }}>
  {slides.map((slide, index) => (
    ...
    >
      ...
      >
        <MDBCardImage
          ...
          onClick={() => navigate('/destination-form', { state: { email: user.email, name: slide.label } })}
        />
      </div>
      <h3
        style={{
    ...
    <MDBRow 
      className="align-items-center mt-5 d-flex justify-content-center" 
  ...
  {promos.map((promo, index) => (
    ...
    >
      ...
      <button 
          type="button" 
          className="btn btn-primary"
          onClick={() => navigate(`/promo/${promo.nav}`, { state: { email: user?.email } })}
          style={{ 
              fontWeight: 'bold',
              fontSize: '14px', 
              width: '80%', 
